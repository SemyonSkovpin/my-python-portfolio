...
def simulationWithDrugV2(startPop, maxPop, maxBirthProb, clearProb, drugs, mutProb, timelyPrescriptions={}, numTrials=10, timeSteps=100):
    ...
    :timelyPrescriptions: {timeStep : str or list(str drugs to add)}
    '''
    # One or more drugs can be scheduled on one timestep, so timelyPrescriptions can contain single strs as well as lists as strs
    # 'Wrap' strs in in timelyPrescriptions into lists
    for timeStep in timelyPrescriptions:
        if type(timelyPrescriptions[timeStep]) == str:
            timelyPrescriptions[timeStep] = [timelyPrescriptions[timeStep]]
...
params = {
    ...
    'mutProb'      : 0.1,
    'drugs'        : ['A', 'B', 'C', 'D', 'E'],
    'timelyPrescriptions' : {100:'A', 200:'B', 300:['C', 'D', 'E']},
