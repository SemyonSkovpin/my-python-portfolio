...
WORDLIST_FILENAME = 'C:/Users/пользователь/OneDrive/AllMyCodeMain/6001x/ProblemSet4/words.txt'
...
def isValidWord(word, hand, wordList):
    ...
    if not word in wordList:
        return False
    wordLettersFrequency = getFrequencyDict(word)
    for letter in wordLettersFrequency:
        if wordLettersFrequency[letter] > hand.get(letter, 0):
            return False
    return True
    ...
    while calculateHandlen(hand):
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print(f'"{str(userInput)}" earned {str(getWordScore(userInput, n))} points. Total: {str(totalScore)} points')
                print()
...
def playGame(wordList):
    ...
    while True:
        ...
        if userInput == 'n':
            lastPlayedHand = dealHand(HAND_SIZE)
            playHand(lastPlayedHand, wordList, HAND_SIZE)
