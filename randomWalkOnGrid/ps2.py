...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        # Tiles occupy range [n, n+1) for each axis where n is number of tile on that axis
...
# === Will random walk robot tend to stay in one area?
def runSingleSimulation(num_robots, speed, width, height, num_steps,
                        robot_type):
    anim = ps2_visualize.RobotVisualization(num_robots, width, height)
    room = RectangularRoom(width, height)
    robots = [robot_type(room, speed) for i in range(num_robots)]
    totalSteps = 0
    while totalSteps < num_steps:
        print (totalSteps / num_steps)
        totalSteps += 1
        for robot in robots:
            robot.updatePositionAndClean()
    anim.update(room, robots)
...
runSingleSimulation(1, 1, 200, 200, 100000, RandomWalkRobot)
